...
export function compileShader(gl, shaderType, source, keywords=null, sourceURL=null) {
    ...
    if(DEBUG)console.log('compileShader:', source);
...
//
// Texture filtering has to match what the device can actually do.
//
// A 32-bit float texture may only use LINEAR filtering where
// OES_texture_float_linear is exposed. Most Android GPUs do not expose it
// (desktop and iOS do), and a float texture set to LINEAR is *incomplete*:
// every sample reads zero, so the simulation renders as a blank canvas with no
// error reported anywhere. Degrade to NEAREST in that case — the simulation is
// unaffected, only the interpolation when the buffer is magnified.
//
// The context setup above already probes the same extensions into
// supportLinearFiltering, but createFBO() only receives `gl`, so it re-probes
// here. getExtension() is cached by the browser, so this is cheap.
function supportedFiltering(gl, type, filtering) {
    const wantsLinear = filtering === gl.LINEAR
        || filtering === gl.LINEAR_MIPMAP_LINEAR
        || filtering === gl.LINEAR_MIPMAP_NEAREST;
    if (!wantsLinear) return filtering;

    const isFloat = type === gl.FLOAT;
    // In WebGL2 half-float textures are filterable without an extension;
    // in WebGL1 they need OES_texture_half_float_linear.
    const isWebGL2 = typeof WebGL2RenderingContext !== 'undefined'
        && gl instanceof WebGL2RenderingContext;
    const isHalfFloat = !isWebGL2 && type === (gl.HALF_FLOAT || 0x8D61);

    if (isFloat && !gl.getExtension('OES_texture_float_linear')) return gl.NEAREST;
    if (isHalfFloat && !gl.getExtension('OES_texture_half_float_linear')) return gl.NEAREST;
    return filtering;
}

export function createFBO(gl, twidth, theight, internalFormat, format, type, filtering, mrtCount = 1) {

    filtering = supportedFiltering(gl, type, filtering);
...
export function printBufferData(gl, buffer, precision=2){
        ...
        for(let iy = 0; iy < height; iy++){
            ...
            for(let ix = 0; ix < width; ix++){
                ...
                let z = data[ind+2];
                let w = data[ind+3];
                line += x.toFixed(precision);line += ' '
                line += y.toFixed(precision);line += ' '
                line += z.toFixed(precision);line += ' '
                line += w.toFixed(precision);line += ';  '
            }
            console.log('',iy,':', line);
        }
        gl.bindFramebuffer(gl.FRAMEBUFFER, null);
