...
export function createDocumentManager(options) {
    ...
    async function readParamsUrl(jsonUrl, overrideParams = null) {
        ...
        // With overrides the document is no longer the preset: name it after
        // what it actually shows, so the title cannot contradict the canvas.
        let docName = overrideParams?.symmetry?.group?.params?.name
                   || getFileNameFromPath(jsonUrl);
        mCurrentDocument = mDocHandler.createDocument({
            appInfo:        mAppInfo,
            params:         mParams,
            thumbMaker:     getThumbMaker(),
